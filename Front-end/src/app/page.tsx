...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-main-bg text-white">
      <Navbar />
      {/*
        IMPORTANT: no `overflow-hidden` here anymore.
        `overflow-hidden` on an ancestor breaks `position: sticky` on any
        descendant — that was why the scroll-driven hero canvas only stuck
        for a moment and then left a blank gap. Decorative glows that need
        clipping now live in their own small overflow-hidden wrapper below,
        scoped only to the sections that need it.
      */}
      <main className="relative w-full flex-1 bg-main-bg">
        <HeroSection
          ...
          heroCtaLink={homePageSettings.heroCtaLink}
          heroCtaTextVal={heroCtaTextVal}
        />
